...
class NeRFDataset_Test:
    def __init__(self, opt, device, downscale=1):
        ...
        if not self.opt.asr:
            ...
            aud_features = torch.tensor(aud_features, dtype=torch.float32)
        ...
        if self.opt.torso_imgs!='':
            if self.preload > 0:
                self.torso_img = torch.tensor(np.stack(self.torso_img, axis=0, dtype=torch.float32)) # [N, H, W, C]
            else:
        ...
        self.poses = torch.tensor(self.poses, dtype=torch.float32) # [N, 4, 4]
        ...
        self.bg_img = torch.tensor(self.bg_img, dtype=torch.float32)
        ...
        if self.opt.exp_eye:
            ...
            self.eye_area = torch.tensor(self.eye_area, dtype=torch.float32).view(-1, 1) # [N, 1]
    ...
    def collate(self, index):
        ...
        if self.opt.torso_imgs!='':
            ...
            if self.preload == 0: # on the fly loading
                ...
                bg_torso_img = torch.tensor(bg_torso_img, dtype=torch.float32).unsqueeze(0)
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1):
        ...
        if self.preload > 0:
            self.images = torch.tensor(np.stack(self.images, axis=0, dtype=torch.float32)) # [N, H, W, C]
            self.torso_img = torch.tensor(np.stack(self.torso_img, axis=0, dtype=torch.float32)) # [N, H, W, C]
        else:
