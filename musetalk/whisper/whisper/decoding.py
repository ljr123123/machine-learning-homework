...
def detect_language(model: "Whisper", mel: Tensor, tokenizer: Tokenizer = None) -> Tuple[Tensor, List[dict]]:
    ...
    n_audio = mel.shape[0]
    x = torch.tensor([[tokenizer.sot]] * n_audio, dtype=torch.float32).to(mel.device)  # [n_audio, 1]
...
class BeamSearchDecoder(TokenDecoder):
    ...
    def update(self, tokens: Tensor, logits: Tensor, sum_logprobs: Tensor) -> Tuple[Tensor, bool]:
        ...
        tokens = torch.tensor(next_tokens, device=tokens.device, dtype=torch.float32)
    ...
    def finalize(self, preceding_tokens: Tensor, sum_logprobs: Tensor):
        ...
        tokens: List[List[Tensor]] = [
            [torch.tensor(seq, dtype=torch.float32) for seq in sequences.keys()] for sequences in self.finished_sequences
        ]
...
class DecodingTask:
    ...
    def run(self, mel: Tensor) -> List[DecodingResult]:
        ...
        tokens: Tensor = torch.tensor([self.initial_tokens], dtype=torch.float32).repeat(n_audio, 1)
